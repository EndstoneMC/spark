...
void testExactRangesAndGap()
{
    ...
    const std::pair<std::uint64_t, std::uint64_t> rows[] = {{0x400, 0x240}, {0x440, 0x260}};
    ...
    CHECK(spark::symbol_guess::dwarf::functionContaining(ranges, 0x460) == nullptr);
}
...
void testDuplicateAndMismatch()
{
    SyntheticImage image = baseImage();
    putFde(image, 0x240, 0x400, 0x20);
    const std::pair<std::uint64_t, std::uint64_t> rows[] = {{0x400, 0x240}, {0x400, 0x240}, {0x440, 0x240}};
    putHeader(image, rows);
