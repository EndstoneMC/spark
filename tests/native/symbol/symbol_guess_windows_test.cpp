...
public:
  ...
  explicit PeFixture(std::size_t size = 0x8000, std::uint64_t load_base = kBase)
    ...
    section(0, ".text", 0x1000, 0x1000,
            IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_EXECUTE | IMAGE_SCN_CNT_CODE);
    section(1, ".rdata", 0x2000, 0x2000,
            IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_INITIALIZED_DATA);
    section(2, ".pdata", 0x4000, 0x1000,
            IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_INITIALIZED_DATA);
    section(3, ".xdata", 0x5000, 0x1000,
            IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_INITIALIZED_DATA);
  }

  template <typename T> void put(std::uint32_t rva, const T &value) {
  ...
  void section(unsigned index, const char *name, std::uint32_t rva,
    ...
    const std::uint32_t offset = static_cast<std::uint32_t>(dos.e_lfanew) +
  ...
  void exceptionDirectory(std::uint32_t rva, std::uint32_t size) {
    ...
    nt.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION] = {rva,
                                                                        size};
    put(0x80, nt);
  }

  void runtimeFunction(unsigned index, std::uint32_t begin, std::uint32_t end,
                       std::uint32_t unwind) {
  ...
  void leafUnwind(std::uint32_t rva, std::uint8_t code_count = 0) {
    putBytes(rva, {1, 0, code_count, 0});
  }

  void chainedUnwind(std::uint32_t rva, std::uint8_t code_count,
                     const RUNTIME_FUNCTION &parent) {
    putBytes(rva, {static_cast<std::uint8_t>((UNW_FLAG_CHAININFO << 3) | 1), 0,
                   code_count, 0});
    const std::uint32_t slots =
        (static_cast<std::uint32_t>(code_count) + 1) & ~1u;
    put(rva + 4 + slots * 2, parent);
  }

  void typeAndCol(std::uint32_t type, std::uint32_t hierarchy,
                  std::uint32_t base_array, std::uint32_t base_descriptor,
                  std::uint32_t col, std::string_view mangled,
  ...
  void vtable(std::uint32_t table, std::uint32_t col,
              std::initializer_list<std::uint32_t> targets) {
  ...
  Engine engine() const {
    return Engine(bytes_.data(), bytes_.size(), load_base_);
  }
...
void addClass(PeFixture &fixture, std::uint32_t base, std::string_view name,
              std::uint32_t table, std::initializer_list<std::uint32_t> targets,
              std::uint32_t offset = 0) {
  fixture.typeAndCol(base, base + 0x100, base + 0x120, base + 0x140,
                     base + 0x180, name, offset);
...
bool testRttiVtableAmbiguity() {
  ...
  CHECK(spark::symbol_guess::windows::chooseVtableLabel(
...
bool testDecodedStringsAndScoring() {
  ...
  CHECK(spark::symbol_guess::windows::scoreStringHint(
            "T *Bedrock::NonOwnerPointer<ChunkPerformanceData>::_get() const") <
        50);
...
bool testInstructionMiddleAndSharedString() {
  {
    ...
    fixture.putBytes(0x1000,
                     {0x48, 0xb8, 0x48, 0x8d, 0x05, 0, 0, 0, 0, 0, 0xc3});
...
bool testLargeRangeLookup() {
  ...
  fixture.section(0, ".text", kText, kPdata - kText,
                  IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_EXECUTE |
                      IMAGE_SCN_CNT_CODE);
  fixture.section(1, ".rdata", 0x100, 0x100,
                  IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_INITIALIZED_DATA);
  fixture.section(2, ".pdata", kPdata, kCount * sizeof(RUNTIME_FUNCTION),
                  IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_INITIALIZED_DATA);
  fixture.section(3, ".xdata", kXdata, 0x1000,
                  IMAGE_SCN_MEM_READ | IMAGE_SCN_CNT_INITIALIZED_DATA);
...
bool testSymbolGuessApplicationPolicy() {
  ...
  CHECK(spark::symbol_guess::windows::guessCurrentModuleSymbols(
            std::span<const std::uint64_t>{})
            .empty());
  CHECK(!spark::symbol_guess::windows::currentModuleStats().initialized);

  CHECK(
      spark::frameMatchesMainModule(0x180001234, 0x1234, 0x180000000, 0x8000));
  CHECK(
      !spark::frameMatchesMainModule(0x180001235, 0x1234, 0x180000000, 0x8000));
  CHECK(
      !spark::frameMatchesMainModule(0x180001234, 0x9234, 0x180000000, 0x8000));
  ...
  spark::applySymbolGuessFallback(library, 0x1234, false,
                                  "vtable: Level::vfn[3]");
...
int evaluateMappedPe(int argc, char **argv) {
  ...
  if (nt.Signature != IMAGE_NT_SIGNATURE ||
      ...
      nt.OptionalHeader.SizeOfImage > 1u << 30) {
  ...
  const std::size_t header_bytes = std::min<std::size_t>(
      {file.size(), mapped.size(), nt.OptionalHeader.SizeOfHeaders});
  ...
  if (section_offset > file.size() ||
      nt.FileHeader.NumberOfSections >
          (file.size() - section_offset) / sizeof(IMAGE_SECTION_HEADER)) {
    std::fprintf(stderr, "invalid section table\n");
    return 2;
  }
  for (WORD i = 0; i < nt.FileHeader.NumberOfSections; ++i) {
    IMAGE_SECTION_HEADER section{};
    std::memcpy(&section, file.data() + section_offset + i * sizeof(section),
                sizeof(section));
    if (section.VirtualAddress >= mapped.size() ||
    ...
    const std::size_t copy = std::min<std::size_t>(
        {section.SizeOfRawData, file.size() - section.PointerToRawData,
         mapped.size() - section.VirtualAddress});
    std::memcpy(mapped.data() + section.VirtualAddress,
                file.data() + section.PointerToRawData, copy);
  }
  ...
  std::fprintf(
      ...
      "bytes=%zu\n",
      stats.function_ranges, stats.chained_ranges, stats.vtables,
      stats.vtable_labels, stats.thunk_resolved, stats.sampled_functions,
      stats.decoded_instructions, stats.string_labels,
      static_cast<unsigned long long>(stats.build_microseconds),
      static_cast<unsigned long long>(stats.batch_microseconds),
...
int main(int argc, char **argv) {
  ...
  if (!testPeAndFunctionRanges() || !testChainedAndMalformedUnwind() ||
      !testDuplicateOverlapAndDeterminism() || !testRttiVtableAmbiguity() ||
      !testInvalidRttiAndThunk() || !testAslrIndependence() ||
      !testDecodedStringsAndScoring() ||
      !testInstructionMiddleAndSharedString() ||
      !testChainedRootStringUniqueness() || !testLargeRangeLookup() ||
      !testSymbolGuessApplicationPolicy()) {
    return 1;
  }
