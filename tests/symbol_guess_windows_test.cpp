...
{
    {
        ...
        CHECK(engine.guess(std::span(&query, 1)).at(query) == "Widget::vfn[0]");
    }
    {
        PeFixture fixture;
        fixture.leafUnwind(0x5000);
        ...
        Engine engine = fixture.engine();
        const std::uint64_t query = 0x1010;
        CHECK(engine.guess(std::span(&query, 1)).at(query) == "Widget::<virtual>");
    }
    {
        PeFixture fixture;
        fixture.leafUnwind(0x5000);
...
bool testInvalidRttiAndThunk()
{
    ...
    CHECK(engine.guess(std::span(&query, 1)).at(query) == "Channel::vfn[0]");
...
bool testAslrIndependence()
{
    ...
    CHECK(preferred.at(0x1010) == "Widget::vfn[0]");
    return true;
}
...
bool testDecodedStringsAndScoring()
{
    ...
    CHECK(guesses.at(query) == "hint: Server Level - tick");
...
bool testChainedRootStringUniqueness()
{
    ...
    CHECK(engine.guess(std::span(&query, 1)).at(query) == "hint: Level - tick chained work");
    return true;
}
...
bool testSymbolGuessApplicationPolicy()
{
    ...
    spark::applySymbolGuessFallback(pdb, 0x1234, true, "Wrong::vfn[0]");
    CHECK(pdb.method_name == "Level::tick");

    spark::ResolvedFrame main;
    spark::applySymbolGuessFallback(main, 0x1234, true, "Level::vfn[3]");
    CHECK(main.method_name == "0x1234 (Level::vfn[3])");

    spark::ResolvedFrame library;
    spark::applySymbolGuessFallback(library, 0x1234, false, "Level::vfn[3]");
