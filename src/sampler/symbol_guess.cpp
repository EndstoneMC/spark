...
public:
    ...
    std::uint64_t ehFrameHdr() const
    {
        return eh_frame_hdr_;
    }

    std::uint64_t ehFrameHdrSize() const
    {
                 ...
                 std::uint64_t &out)
{
    ...
    case kDwEhPeSleb128:
    default:
        return false;  // absptr and LEB128 forms do not appear in a binary search table
    }
...
// by initial location. Each entry gives a function start; the extent runs to the
// next start, which is what functionContaining needs. The FDE's own length is not
// decoded because .eh_frame parsing is far more fragile than this table.
void collectFunctions(const ImageView &img, GuessTable &table)
{
...
// [offset-to-top][typeinfo*][slot 0][slot 1]..., so a candidate is a pointer to a
// valid type_info followed by pointers into executable code. Each slot's target
// function is labelled "Class::vfn[i]".
void collectVtableLabels(const ImageView &img, GuessTable &table)
{
    for (const Section &section : img.sections()) {
        if (section.executable) {
