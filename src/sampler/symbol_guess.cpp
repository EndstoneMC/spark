...
template <typename T> bool readAt(const ImageView &img, std::uint64_t rva, T &out)
{
...
bool readEncoded(const ImageView &img, std::uint8_t encoding, std::uint64_t base, std::uint64_t &rva,
                 std::uint64_t &out)
{
...
void collectFunctions(const ImageView &img, GuessTable &table)
{
    ...
    for (std::size_t i = 0; i < table.ranges.size(); ++i) {
        ...
        const std::uint64_t section_end = section != nullptr ? section->end : range.begin;
        const std::uint64_t next = i + 1 < table.ranges.size() ? table.ranges[i + 1].begin : section_end;
        range.end = std::min(next, section_end);
    }
    table.ranges.erase(std::remove_if(table.ranges.begin(), table.ranges.end(),
...
// Scans data segments for Itanium vtables. The layout is
// [offset-to-top][typeinfo*][slot 0][slot 1]..., so a candidate is a pointer to
// a valid type_info followed by pointers into executable code. Each slot's
// target function is labelled "Class::vfn[i]".
void collectVtableLabels(const ImageView &img, GuessTable &table)
{
    for (const Section &section : img.sections()) {
        ...
        for (std::uint64_t rva = (section.begin + 7u) & ~7ull; rva + 24 <= section.end; rva += 8) {
...
// Scans executable segments for RIP-relative LEA instructions whose target is a
...
{
    std::unordered_map<std::uint64_t, StringReference> references;
    for (const Section &section : img.sections()) {
        if (!section.executable) {
            continue;
        }
        const std::uint8_t *bytes = img.at(0);
        for (std::uint64_t rva = section.begin; rva + 7 <= section.end; ++rva) {
            if (bytes[rva] < 0x48 || bytes[rva] > 0x4f || bytes[rva + 1] != 0x8d || (bytes[rva + 2] & 0xc7) != 0x05) {
                continue;
            }
            ...
            references.emplace(target, StringReference{fn->root, static_cast<std::uint32_t>(value.size()), false});
        }
    }
...
std::unordered_map<std::uint64_t, std::string> guessMainModuleSymbols(std::span<const std::uint64_t> rvas)
{
    ...
    for (std::uint64_t rva : rvas) {
        std::string label = guessMainModuleSymbol(rva);
        if (!label.empty()) {
            out.emplace(rva, std::move(label));
        }
    }
    return out;
}
...
std::unordered_map<std::uint64_t, std::string> guessMainModuleSymbols(std::span<const std::uint64_t>) { return {}; }
