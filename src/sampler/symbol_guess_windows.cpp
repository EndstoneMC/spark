...
std::string chooseVtableLabel(std::vector<VtableEvidence> evidence)
{
    ...
    if (slots.size() == 1) {
        return class_name + "::vfn[" + std::to_string(*slots.begin()) + "]";
    }
    return class_name + "::<virtual>";
}
...
std::string formatStringHint(std::string_view value)
{
    ...
    std::string out(value.substr(0, kMaximum));
    if (value.size() > kMaximum) {
        out.resize(kMaximum - 3);
        out += "...";
    }
    return "hint: " + out;
}
...
struct Engine::Impl {
    ...
    void collectVtables()
    {
        ...
        for (auto &[root, evidence] : candidates) {
            std::string label = chooseVtableLabel(evidence);
            if (label.empty()) {
        ...
        for (const auto &[root, candidates] : string_candidates) {
            ...
            for (const StringCandidate &candidate : candidates) {
                ...
                if (refs != references.end() && refs->second.size() == 1 && *refs->second.begin() == root) {
                    label = formatStringHint(candidate.value);
                    break;
                }
...
bool Engine::valid() const { return impl_ != nullptr && impl_->stats.initialized; }
...
std::unordered_map<std::uint64_t, std::string> Engine::guess(std::span<const std::uint64_t> rvas)
{
    return impl_ != nullptr ? impl_->guess(rvas) : std::unordered_map<std::uint64_t, std::string>{};
}
