...
std::string classNameFromTypeDescriptor(std::string_view mangled) {
  ...
  if (std::any_of(parts.begin(), parts.end(),
                  [](std::string_view part) { return part.empty(); })) {
    return {};
  }
  std::string out;
...
struct Engine::Impl {
  ...
  template <typename T> bool readRaw(std::size_t offset, T &out) const {
  ...
  const Section *sectionContaining(std::uint32_t rva,
                                   std::uint32_t length = 1) const {
    auto it = std::upper_bound(sections.begin(), sections.end(), rva,
                               [](std::uint32_t value, const Section &section) {
                                 return value < section.begin;
                               });
    if (it == sections.begin()) {
      return nullptr;
    }
    --it;
    return rva >= it->begin && rva < it->end && length <= it->end - rva
               ? &*it
               : nullptr;
  }

  template <typename T> bool read(std::uint32_t rva, T &out) const {
    if (sectionContaining(rva, static_cast<std::uint32_t>(sizeof(T))) ==
        nullptr) {
      return false;
    }
  ...
  bool parseHeaders() {
    ...
    if (!readRaw(0, dos) || dos.e_magic != IMAGE_DOS_SIGNATURE ||
        dos.e_lfanew < 0) {
      return false;
    }
    ...
    const std::size_t section_offset = nt_offset + sizeof(DWORD) +
    ...
    for (WORD i = 0; i < nt.FileHeader.NumberOfSections; ++i) {
      ...
      std::uint32_t end = 0;
      if (!checkedAdd(header.VirtualAddress, header.Misc.VirtualSize, end) ||
          end > image_size || header.VirtualAddress >= end) {
        continue;
      }
      sections.push_back(
  ...
  bool validFunction(const RUNTIME_FUNCTION &function) const {
    ...
    const Section *code = sectionContaining(
        function.BeginAddress, function.EndAddress - function.BeginAddress);
    ...
    return version == 1 && (flags & ~(UNW_FLAG_EHANDLER | UNW_FLAG_UHANDLER |
                                      UNW_FLAG_CHAININFO)) == 0;
  }
  ...
  std::optional<std::uint32_t> chainRoot(const RUNTIME_FUNCTION &start) const {
    ...
    for (unsigned depth = 0; depth < 32; ++depth) {
      if (!validFunction(function) ||
          !seen.emplace(function.BeginAddress, function.EndAddress,
                        function.UnwindData)
               .second) {
        return std::nullopt;
      }
      ...
      const std::uint32_t slots =
          (static_cast<std::uint32_t>(header[2]) + 1u) & ~1u;
      std::uint32_t chained_rva = 0;
      if (!checkedAdd(function.UnwindData, 4u + 2u * slots, chained_rva) ||
          !read(chained_rva, function)) {
        return std::nullopt;
      }
    }
    return std::nullopt;
  ...
  void collectFunctions() {
    if (exception.VirtualAddress == 0 ||
        exception.Size < sizeof(RUNTIME_FUNCTION) ||
        exception.Size % sizeof(RUNTIME_FUNCTION) != 0 ||
        sectionContaining(exception.VirtualAddress, exception.Size) ==
            nullptr) {
      return;
    }
    ...
    for (std::uint32_t i = 0; i < count; ++i) {
      ...
      if (!read(exception.VirtualAddress + i * sizeof(RUNTIME_FUNCTION),
                function)) {
    ...
    while (cluster_begin < ranges.size()) {
      ...
      while (cluster_end < ranges.size() &&
             ranges[cluster_end].begin < maximum_end) {
  ...
  const FunctionRange *containing(std::uint64_t rva) const {
    ...
    auto it = std::upper_bound(
        ranges.begin(), ranges.end(), static_cast<std::uint32_t>(rva),
  ...
  std::optional<std::pair<std::string, CompleteObjectLocator>>
  validateCol(std::uint32_t rva) const {
    CompleteObjectLocator col{};
    if (!read(rva, col) || col.signature != 1 || col.self != rva) {
      return std::nullopt;
    }
    ClassHierarchyDescriptor hierarchy{};
    if (!read(col.class_descriptor, hierarchy) || hierarchy.signature != 0 ||
        hierarchy.base_count == 0 || hierarchy.base_count > 1024) {
      return std::nullopt;
    }
    const Section *base_array =
        sectionContaining(hierarchy.base_array, hierarchy.base_count * 4u);
    ...
    if (!read(hierarchy.base_array, first_base) ||
        !read(first_base, first_type) ||
        !read(first_base + 4, contained_bases) ||
        first_type != col.type_descriptor ||
    ...
    return std::pair<std::string, CompleteObjectLocator>{std::move(class_name),
                                                         col};
  }
  ...
  std::optional<std::uint32_t> directThunkTarget(std::uint32_t rva) {
    ...
    const std::uint32_t available =
        std::min<std::uint32_t>(section->end - cursor, 24);
    if (available < 5) {
      return std::nullopt;
    }
    const std::uint8_t *code = image + cursor;
    std::uint32_t prefix = 0;
    if (available >= 9 && code[0] == 0x48 && code[1] == 0x83 &&
        (code[2] == 0xe9 || code[2] == 0xc1)) {
      prefix = 4;
    } else if (available >= 12 && code[0] == 0x48 && code[1] == 0x81 &&
               (code[2] == 0xe9 || code[2] == 0xc1)) {
      prefix = 7;
    } else if (available >= 9 && code[0] == 0x48 && code[1] == 0x8d &&
               code[2] == 0x49) {
      prefix = 4;
    } else if (available >= 12 && code[0] == 0x48 && code[1] == 0x8d &&
               code[2] == 0x89) {
      prefix = 7;
    }
    ...
    const std::int64_t target =
        static_cast<std::int64_t>(rva) + prefix + 5 + displacement;
    if (target < 0 || target > std::numeric_limits<std::uint32_t>::max()) {
      return std::nullopt;
    }
    const FunctionRange *function =
        containing(static_cast<std::uint32_t>(target));
    if (function == nullptr) {
      return std::nullopt;
    }
    ...
    for (const Section &section : sections) {
      ...
      for (std::uint32_t rva = start; rva <= section.end - 16u; rva += 8) {
        ...
        for (std::uint32_t slot = 0;
             slot < 512 && table <= section.end - 8u * (slot + 1u); ++slot) {
          ...
          candidates[root].push_back(
              {class_name, slot, col.offset != 0, via_thunk});
  ...
  const FunctionRange *fragmentContaining(std::uint32_t root,
                                          std::uint32_t rva) const {
    const FunctionRange *range = containing(rva);
    return range != nullptr && range->root == root ? range : nullptr;
  }

  std::vector<StringCandidate> decodeStrings(std::uint32_t root,
                                             BuildStats &batch) const {
    const FunctionRange *root_range = containing(root);
    if (root_range == nullptr || root_range->begin != root ||
        root_range->root != root) {
      return {};
    }
    std::vector<std::uint32_t> work{root};
    if (const auto fragments = chained_fragment_starts.find(root);
        fragments != chained_fragment_starts.end()) {
      work.insert(work.end(), fragments->second.begin(),
                  fragments->second.end());
    }
      ...
      while (const FunctionRange *fragment = fragmentContaining(root, cursor)) {
        ...
        for (unsigned i = 0; i < used; ++i) {
          ...
          const std::uint32_t address =
              static_cast<std::uint32_t>(instruction.addr);
          ...
          if (instruction.opcode == I_LEA &&
            ...
            const std::uint64_t target_wide =
            ...
            if (target_wide <= std::numeric_limits<std::uint32_t>::max()) {
              ...
              if (score >= ::spark::symbol_guess::kMinimumStringHintScore) {
                candidates.try_emplace(
                    target, StringCandidate{target, std::move(value), score});
              }
            }
          }
          ...
          if (flow == FC_CND_BRANCH || flow == FC_UNC_BRANCH) {
            for (const _Operand &operand : instruction.ops) {
              if (operand.type == O_PC) {
                const std::uint64_t target_wide =
                    INSTRUCTION_GET_TARGET(&instruction);
                if (target_wide <= std::numeric_limits<std::uint32_t>::max() &&
                    fragmentContaining(root, static_cast<std::uint32_t>(
                                                 target_wide)) != nullptr) {
  ...
  void scanCandidateReferences(
    ...
    for (const Section &section : sections) {
      ...
      for (std::uint32_t rva = section.begin; rva <= section.end - 7; ++rva) {
        ...
        if (code[0] < 0x48 || code[0] > 0x4f || code[1] != 0x8d ||
            (code[2] & 0xc7) != 0x05) {
          continue;
        }
        std::int32_t displacement = 0;
        std::memcpy(&displacement, code + 3, sizeof(displacement));
        const std::int64_t target_wide =
            static_cast<std::int64_t>(rva) + 7 + displacement;
        if (target_wide < 0 ||
            target_wide > std::numeric_limits<std::uint32_t>::max()) {
          continue;
        }
        const auto target = static_cast<std::uint32_t>(target_wide);
  ...
  void updateApproximateBytes() {
    std::size_t bytes = ranges.capacity() * sizeof(FunctionRange) +
  ...
  guess(std::span<const std::uint64_t> rvas) {
    ...
    for (const auto &[root, inputs] : root_inputs) {
      if (const auto label = vtable_labels.find(root);
          label != vtable_labels.end()) {
    ...
    for (const auto &[root, candidates] : string_candidates) {
      ...
      for (const StringCandidate &candidate : candidates) {
        ...
        if (refs != references.end() && refs->second.size() == 1 &&
          ...
          label = ::spark::symbol_guess::formatStringHint(candidate.value,
                                                          candidate.score);
          break;
        }
...
Engine::Engine(const std::uint8_t *image, std::size_t mapped_size,
...
Engine &currentEngine() {
  ...
  state.engine = std::make_unique<Engine>([] {
    ...
    return Engine(static_cast<const std::uint8_t *>(info.lpBaseOfDll),
                  info.SizeOfImage,
