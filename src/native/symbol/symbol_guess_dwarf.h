...
public:
    ...
    virtual bool read(std::uint64_t rva, void *out, std::size_t length) const = 0;
    virtual bool executable(std::uint64_t rva, std::size_t length) const = 0;
    // End of readable mapped segment containing rva; used for sequential FDE recovery.
...
// Parses and validates the GNU .eh_frame_hdr search table. Function ends come from FDE address_range;
// malformed or overlapping entries are rejected conservatively.
