// Windows CNG/BCrypt implementation of the Crypto interface.
// Replaces OpenSSL on Windows.  The wire-protocol key formats
// (X.509 SubjectPublicKeyInfo DER for public keys, PKCS#8 DER for
// private keys) are identical to the OpenSSL backend so that
// signatures and keys interoperate across platforms.
...
void encodeTLV(std::vector<std::uint8_t> &out, std::uint8_t tag,
               const std::uint8_t *content, std::size_t len) {
    out.push_back(tag);
...
void encodeInteger(std::vector<std::uint8_t> &out,
...
void encodeOctetString(std::vector<std::uint8_t> &out,
                       const std::vector<std::uint8_t> &content) {
...
constexpr std::uint8_t kRsaAlgId[] = {
    0x30, 0x0d,
...
const std::uint8_t *parseElement(const std::uint8_t *data, std::size_t len,
                                 std::uint8_t expectedTag,
                                 const std::uint8_t **content,
                                 std::size_t *contentLen) {
    if (len < 2 || data[0] != expectedTag) return nullptr;
    std::size_t i = 1;
    std::size_t hdrLen;
    if (data[i] < 0x80) {
        *contentLen = data[i];
        ...
        ++i;
        if (n == 0 || n > 4 || len < i + n) return nullptr;
        *contentLen = 0;
    ...
    if (len < hdrLen + *contentLen) return nullptr;
    *content = data + hdrLen;
...
bool parseInteger(const std::uint8_t *data, std::size_t len,
...
std::vector<std::uint8_t> buildRsaKeyBlob(bool is_private,
                                          const RsaKeyParams &p) {
...
bool parseRsaKeyBlob(const std::uint8_t *blob, std::size_t len,
                     RsaKeyParams &params, bool &is_private) {
    if (len < sizeof(BCRYPT_RSAKEY_BLOB)) return false;
    ...
    if (!extract(hdr->cbPublicExp, params.public_exp)) return false;
    if (!extract(hdr->cbModulus, params.modulus)) return false;
    if (is_private) {
        if (!extract(hdr->cbPrime1, params.prime1)) return false;
        if (!extract(hdr->cbPrime2, params.prime2)) return false;
        if (!extract(hdr->cbPrime1, params.exponent1)) return false;
        if (!extract(hdr->cbPrime2, params.exponent2)) return false;
        if (!extract(hdr->cbPrime1, params.coefficient)) return false;
        if (!extract(hdr->cbModulus, params.private_exp)) return false;
    }
    return true;
}
...
std::vector<std::uint8_t> publicBlobToX509(const std::uint8_t *blob, std::size_t len) {
    RsaKeyParams params;
    bool is_private;
    if (!parseRsaKeyBlob(blob, len, params, is_private) || is_private) return {};
...
std::vector<std::uint8_t> x509ToPublicBlob(const std::uint8_t *data, std::size_t len) {
    ...
    std::size_t alg_len;
    const std::uint8_t *after_alg = der::parseElement(content, contentLen, 0x30,
                                                       &alg_content, &alg_len);
    if (!after_alg) return {};
    ...
    std::size_t bit_len;
    const std::uint8_t *after_bit = der::parseElement(after_alg,
                                                       contentLen - (after_alg - content),
                                                       0x03, &bit_content, &bit_len);
    if (!after_bit || bit_len < 1) return {};
    ...
    std::size_t rsa_len;
    const std::uint8_t *after_rsa = der::parseElement(bit_content, bit_len, 0x30,
                                                       &rsa_content, &rsa_len);
    if (!after_rsa) return {};
    ...
    if (!der::parseInteger(cur, cur_len, params.modulus, &cur)) return {};
    cur_len = rsa_len - (cur - rsa_content);
    if (!der::parseInteger(cur, cur_len, params.public_exp, &cur)) return {};
...
std::vector<std::uint8_t> pkcs8ToPrivateBlob(const std::uint8_t *data, std::size_t len) {
    ...
    if (!der::parseInteger(content, contentLen, version, &after_ver)) return {};

    // Skip AlgId (SEQUENCE).
    const std::uint8_t *alg_content;
    std::size_t alg_len;
    const std::uint8_t *after_alg = der::parseElement(after_ver,
                                                       contentLen - (after_ver - content),
                                                       0x30, &alg_content, &alg_len);
    if (!after_alg) return {};

    // Parse OCTET STRING containing RSAPrivateKey.
    const std::uint8_t *oct_content;
    std::size_t oct_len;
    const std::uint8_t *after_oct = der::parseElement(after_alg,
                                                       contentLen - (after_alg - content),
                                                       0x04, &oct_content, &oct_len);
    if (!after_oct) return {};

    // RSAPrivateKey = SEQUENCE { version, n, e, d, p, q, dmp1, dmq1, iqmp }
    const std::uint8_t *rsa_content;
    std::size_t rsa_len;
    const std::uint8_t *after_rsa = der::parseElement(oct_content, oct_len, 0x30,
                                                       &rsa_content, &rsa_len);
    if (!after_rsa) return {};
    ...
    // Skip version.
    if (!der::parseInteger(cur, cur_len, version, &cur)) return {};
    cur_len = rsa_len - (cur - rsa_content);
    if (!der::parseInteger(cur, cur_len, params.modulus, &cur)) return {};
    cur_len = rsa_len - (cur - rsa_content);
    if (!der::parseInteger(cur, cur_len, params.public_exp, &cur)) return {};
    cur_len = rsa_len - (cur - rsa_content);
    if (!der::parseInteger(cur, cur_len, params.private_exp, &cur)) return {};
    cur_len = rsa_len - (cur - rsa_content);
    if (!der::parseInteger(cur, cur_len, params.prime1, &cur)) return {};
    cur_len = rsa_len - (cur - rsa_content);
    if (!der::parseInteger(cur, cur_len, params.prime2, &cur)) return {};
    cur_len = rsa_len - (cur - rsa_content);
    if (!der::parseInteger(cur, cur_len, params.exponent1, &cur)) return {};
    cur_len = rsa_len - (cur - rsa_content);
    if (!der::parseInteger(cur, cur_len, params.exponent2, &cur)) return {};
    cur_len = rsa_len - (cur - rsa_content);
    if (!der::parseInteger(cur, cur_len, params.coefficient, &cur)) return {};
...
std::vector<std::uint8_t> sha256(const std::uint8_t *data, std::size_t len) {
    ...
    BCryptGetProperty(alg, BCRYPT_HASH_LENGTH,
                      reinterpret_cast<PUCHAR>(&hash_len), sizeof(DWORD), &cbData, 0);
    BCryptGetProperty(alg, BCRYPT_OBJECT_LENGTH,
                      reinterpret_cast<PUCHAR>(&hash_obj_len), sizeof(DWORD), &cbData, 0);

    std::vector<std::uint8_t> hash_obj(hash_obj_len);
    std::vector<std::uint8_t> hash(hash_len);
    BCRYPT_HASH_HANDLE hash_handle = nullptr;
    if (BCryptCreateHash(alg, &hash_handle, hash_obj.data(), hash_obj_len,
                         nullptr, 0, 0) == 0) {
        BCryptHashData(hash_handle, const_cast<PUCHAR>(data),
...
Crypto::KeyPair Crypto::generateKeyPair() {
    ...
    BCryptExportKey(key, nullptr, BCRYPT_RSAPUBLIC_BLOB, nullptr, 0,
                    &pub_blob_len, 0);
    std::vector<std::uint8_t> pub_blob(pub_blob_len);
    if (BCryptExportKey(key, nullptr, BCRYPT_RSAPUBLIC_BLOB,
                        pub_blob.data(), pub_blob_len, &pub_blob_len, 0) == 0) {
    ...
    BCryptExportKey(key, nullptr, BCRYPT_RSAFULLPRIVATE_BLOB, nullptr, 0,
                    &priv_blob_len, 0);
    std::vector<std::uint8_t> priv_blob(priv_blob_len);
    if (BCryptExportKey(key, nullptr, BCRYPT_RSAFULLPRIVATE_BLOB,
                        priv_blob.data(), priv_blob_len, &priv_blob_len, 0) == 0) {
...
std::vector<std::uint8_t> Crypto::sign(const std::vector<std::uint8_t> &private_key_pkcs8,
    ...
    if (blob.empty()) return signature;
    ...
    BCRYPT_KEY_HANDLE key = nullptr;
    if (BCryptImportKeyPair(alg, nullptr, BCRYPT_RSAFULLPRIVATE_BLOB, &key,
    ...
    DWORD sig_len = 0;
    if (BCryptSignHash(key, &pad_info, hash.data(),
                       static_cast<ULONG>(hash.size()),
                       nullptr, 0, &sig_len, BCRYPT_PAD_PKCS1) == 0 && sig_len > 0) {
...
bool Crypto::verify(const std::vector<std::uint8_t> &public_key_x509,
                    const std::uint8_t *message, std::size_t length,
    ...
    if (!hash.empty()) {
        BCRYPT_PKCS1_PADDING_INFO pad_info = {};
        pad_info.pszAlgId = BCRYPT_SHA256_ALGORITHM;
        ok = BCryptVerifySignature(key, &pad_info, hash.data(),
                                   static_cast<ULONG>(hash.size()),
                                   const_cast<PUCHAR>(signature),
                                   static_cast<ULONG>(sig_length),
                                   BCRYPT_PAD_PKCS1) == 0;
    }
